...
impl From<GrabMode> for u32 {
    fn from(value: GrabMode) -> Self {
        match value {
            GrabMode::CameraGrabWhenEmpty => camera::camera_grab_mode_t_CAMERA_GRAB_WHEN_EMPTY,
            GrabMode::CameraGrabLatest => camera_grab_mode_t_CAMERA_GRAB_LATEST,
        }
    }
}
...
impl<'a> Camera<'a> {
    pub fn new(
        ...
        let config = camera::camera_config_t {
            ...
            // I needed to decrease frequency to avoid frequent JPEG corruption
            ...
            // Larger image size leads to large amount of coruption
            frame_size: camera::framesize_t_FRAMESIZE_SXGA,
